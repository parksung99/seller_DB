...
alter table public.beauty_seller_candidates
  ...
  add column if not exists matched_hashtags_count integer default 0,
  add column if not exists matched_beauty_keywords text,
  add column if not exists matched_selling_keywords text,
  add column if not exists negative_keywords text,
  add column if not exists dm_available text,
...
-- remove duplicate seller_id rows (keep latest by updated_at)
with ranked as (
  select id,
         row_number() over (
